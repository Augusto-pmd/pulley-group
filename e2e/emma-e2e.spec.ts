...
test.describe('Emma - E2E CRUD', () => {
  ...
  test('debe iniciar fondo y crear movement real', async ({ page }) => {
    ...
    // Llenar formulario de inicio - usar selectores más específicos
    // Capital inicial (primer input con inputmode="decimal")
    ...
    // Tasa esperada (input después del capital, sin inputmode)
    const tasaInput = page.locator('form[data-testid="modal-form"] input[type="text"]:not([inputmode])').nth(1);
    await tasaInput.waitFor({ state: 'visible', timeout: 5000 });
    await tasaInput.fill('10');
    
    // Horizonte (último input de texto, requerido)
    ...
    // Submit
    const submitButton = page.locator('form[data-testid="modal-form"] button[type="submit"], form[data-testid="modal-form"] button:has-text("Iniciar fondo")').first();
